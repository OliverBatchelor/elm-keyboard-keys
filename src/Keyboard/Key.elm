...
{-| Higher level Key data type for keyboard events.
...
onKeyPress toMsg =
    Keyboard.presses (toMsg << fromCode)
...
{-| Key data type. Originally from keyboard-extra package.
-}
type Key
    = Cancel
    | Help
...
keyDict : Dict String KeyCode
keyDict =
  Dict.fromList <| List.map (\(code, key) -> (toString key, code)) codeBook
